...
const phases = {
  build: {namespace:'agri-nmp-tools', name: `${name}`, phase: 'build', changeId:changeId, suffix: `-build-${changeId}`, instance: `${name}-build-${changeId}`, version:`${version}-${changeId}`, tag:`build-${version}-${changeId}`, weasyprintreplicas:'1', nmpreplicas:'1', pdfreplicas:'1' , persistentVolumeClass:'netapp-file-standard' , certbotManaged:'false', host:`${name}-pr-${changeId}-agri-nmp-tools.pathfinder.gov.bc.ca`},
   ...
   test: {namespace:'agri-nmp-test' , name: `${name}`, phase: 'test' , changeId:changeId, suffix: `-test`             , instance: `${name}-test`             , version:`${version}-${changeId}`, tag:`test-${version}`             , weasyprintreplicas:'2', nmpreplicas:'2', pdfreplicas:'2' , persistentVolumeClass:'netapp-file-standard' , certbotManaged:'false', host:`${name}-test-agri-nmp-test.pathfinder.gov.bc.ca`},
   prod: {namespace:'agri-nmp-prod' , name: `${name}`, phase: 'prod' , changeId:changeId, suffix: `-prod`             , instance: `${name}-prod`             , version:`${version}-${changeId}`, tag:`prod-${version}`             , weasyprintreplicas:'3', nmpreplicas:'3', pdfreplicas:'3' , persistentVolumeClass:'netapp-file-standard' , certbotManaged:'true',  host:'nmp.apps.nrs.gov.bc.ca'}
};
