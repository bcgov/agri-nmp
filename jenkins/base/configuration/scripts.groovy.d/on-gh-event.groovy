        ...
        try{
            if ("pull_request" == ghEventType){
                ...
                if ("closed" == payload.action){
                    ...
                    println exec(['oc', '--namespace=agri-nmp-tools', 'delete', 'all', '-l', "app=nmp-pr-${payload.number}"])
                    println exec(['oc', '--namespace=agri-nmp-dev', 'delete', 'all', '-l', "app=nmp-dev-pr-${payload.number}"])
                    println exec(['oc', '--namespace=agri-nmp-test', 'delete', 'all', '-l', "app=nmp-test-pr-${payload.number}"])
